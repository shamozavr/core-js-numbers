...
function roundToPowerOfTen(num, pow) {
  ...
  const addition = Math.round(remainder / 10 ** pow) * 10 ** pow;
  const integer = num - remainder + addition;
  return integer;
}
...
function getFibonacciNumber(index) {
  ...
  for (let i = 0; arrOfFibonacci.length < index + 1; i += 1) {
    arrOfFibonacci.push(
      arrOfFibonacci[arrOfFibonacci.length - 2] +
        arrOfFibonacci[arrOfFibonacci.length - 1]
    );
  }
  return arrOfFibonacci[index];
}
...
function getSumOfNumbers(x1, x2, x3) {
  ...
  return (x1 + x2 + x3).toPrecision(precision);
}
